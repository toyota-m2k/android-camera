...
object Uploader : ProgressWorkerProcessor() {
    ...
    fun workDataFromItem(db:ScDB, item: ItemEx):Data {
        return workDataOf(
            ...
            KEY_FILE_PATH to db.fileOf(item).absolutePath,
            KEY_DURATION to "${item.duration}",
        )
    }

    class ULWorker(context: Context, params: WorkerParameters) : ProgressWorker(context, params) {
        override suspend fun doWork(): Result {
            ...
            return try {
                ...
                val request = Request.Builder()
                    .url("http://${Authentication.activeHostAddress}/slot$slot/upload")
                    .post(multipartBody)
                    .build()
                ...
                if (code == 200) {
                    ...
                    MetaDB.withDB { db-> db.updateCloud(originalId.toInt(), CloudStatus.Uploaded)}
                    Result.success()
                } else {
    ...
    fun upload(context: Context, db:ScDB, item:ItemEx, progress:((current:Long, total:Long)->Unit)?):IAwaiter<Boolean> {
        return process<ULWorker>(context, workDataFromItem(db, item), progress)
    }
}