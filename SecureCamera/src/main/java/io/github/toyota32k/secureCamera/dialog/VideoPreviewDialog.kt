...
import io.github.toyota32k.secureCamera.databinding.DialogVideoPreviewBinding
        ...
        companion object {
            fun create(task: IUtImmortalTask, uri:String, name:String, chapters: List<IChapter>?, setupPlayer:((PlayerControllerModel.Builder)->Unit)?): VideoPreviewViewModel {
                return task.createViewModel<VideoPreviewViewModel> {
                    ...
                    val builder = PlayerControllerModel.Builder(SCApplication.instance, viewModelScope)
                        .autoPlay(true)
                    if (chapterList!=null) {
